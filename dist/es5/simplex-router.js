...
var getSearchPathParameters = function getSearchPathParameters(searchParamsPath) {
  ...
  if (searchParamsPath) {
    var pairs = (searchParamsPath[0] === '?' ? searchParamsPath.substr(1) : searchParamsPath).split('&');
...
function () {
  function SimplexRouter(routeTemplates, compileOptions) {
    ...
    for (var templateIndex = 0; templateIndex < templates.length; templateIndex++) {
      ...
      var templatePath = options.templateKey ? options.templateKey(template) : typeof template === 'string' ? template : null;
  ...
  _createClass(SimplexRouter, [{
    ...
    value: function match(pathToMatchTemplates, options) {
      ...
      var searchParams = getSearchPathParameters(splitPathFromSearchParams[1], options.decode);
        ...
        for (var pathForMatchVariantsIndex = 0; pathForMatchVariantsIndex < pathForMatchVariants.length; pathForMatchVariantsIndex++) {
          ...
          if (pathForMatchVariantMatch) {
            ...
            for (var compiledRouteParamIndex = 0; compiledRouteParamIndex < compiledRoute.templateParameterNames.length; compiledRouteParamIndex++) {
              var key = compiledRoute.templateParameterNames[compiledRouteParamIndex];
              var value = pathForMatchVariantMatch[compiledRouteParamIndex + 1] || '';
