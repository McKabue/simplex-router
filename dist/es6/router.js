import { defaultCompileOptions, quoteRegExp, getSearchPathParameters } from './utils';
export class SimplexRouter {
    ...
    constructor(routeTemplates, compileOptions) {
        ...
        const templates = this.routeTemplates = Array.isArray(routeTemplates)
            ? routeTemplates
            : [routeTemplates];
        const compiledRouteTemplates = (this.compiledRouteTemplates = []);
        for (let templateIndex = 0; templateIndex < templates.length; templateIndex++) {
            const template = templates[templateIndex];
            const templatePath = options.templateKey
    ...
    // eslint-disable-next-line complexity
    match(pathToMatchTemplates, options) {
        const optionsWithDefaults = Object.assign({ onlyFirstTemplate: false, decode: true }, options);
        ...
        const splitPathFromSearchParams = pathToMatchTemplates.split('?');
        const pathWithoutSearchParams = splitPathFromSearchParams[0];
        const searchParams = getSearchPathParameters(splitPathFromSearchParams[1], optionsWithDefaults.decode);
        for (let compiledRouteIndex = 0; compiledRouteIndex < this.compiledRouteTemplates.length; compiledRouteIndex++) {
            ...
            for (let pathForMatchVariantsIndex = 0; pathForMatchVariantsIndex < pathForMatchVariants.length; pathForMatchVariantsIndex++) {
                ...
                if (pathForMatchVariantMatch) {
                    ...
                    let params = {};
                    for (let compiledRouteParamIndex = 0; compiledRouteParamIndex
                        < compiledRoute.templateParameterNames.length; compiledRouteParamIndex++) {
                    ...
                    if (typeof compiledRoute.template === 'string') {
                        ...
                        templateMatchResponse = Object.assign(Object.assign({}, compiledRoute.template), { params: params });
                    }
                    if (optionsWithDefaults.onlyFirstTemplate) {
