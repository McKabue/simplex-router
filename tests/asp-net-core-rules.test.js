...
const match = (rule, index) => {
    ...
    if (!test) {
        throw new Error(`The test "${rule.test}" is matched NOT matched by "${rule.match}"`);
    }
    ...
    const matchedIndex = aspNetCoreRules.findIndex(i => i.match.test(rule.test));
    if (matchedIndex !== index) {
        throw new Error(`The test "${rule.test}" is matched by a different route, "${aspNetCoreRules[matchedIndex].match}"`);
    }
    ...
    const matcherIndex = aspNetCoreRules.findIndex(i => rule.match.test(i.test));
    if (matcherIndex != index) {
        throw new Error(`The test "${rule.match}" is matching another test, "${aspNetCoreRules[matcherIndex].test}"`);
    }
